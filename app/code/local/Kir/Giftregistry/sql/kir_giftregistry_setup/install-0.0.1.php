...
if ($installer->getConnection()->isTableExists($tableName) != true) {
    $table = $installer->getConnection()
        ->newTable($installer->getTable('kir_giftregistry/type'))
 ...
 *
 */
$tableName = $installer->getTable('kir_giftregistry/entity');
// Check if the table already exists
if ($installer->getConnection()->isTableExists($tableName) != true) {
    $table = $installer->getConnection()
        ->newTable($tableName)
        ...
        ->addColumn('created_at', Varien_Db_Ddl_Table::TYPE_TIMESTAMP, null,
            ...
            'Created At')
        ->addIndex($installer->getIdxName('kir_giftregistry/entity', array('customer_id')),
            array('customer_id'))
        ->addIndex($installer->getIdxName('kir_giftregistry/entity', array('website_id')),
            array('website_id'))
        ->addIndex($installer->getIdxName('kir_giftregistry/entity', array('type_id')),
            array('type_id'))
        ->addForeignKey(
            $installer->getFkName(
                'kir_giftregistry/entity',
                'customer_id',
                'customer/entity',
                'entity_id'
            ),
        ...
        ->addForeignKey(
            $installer->getFkName(
                'kir_giftregistry/entity',
                'type_id',
                'kir_giftregistry/type',
                'type_id'
            ),
            'type_id', $installer->getTable('kir_giftregistry/type'), 'type_id',
            Varien_Db_Ddl_Table::ACTION_CASCADE, Varien_Db_Ddl_Table::ACTION_CASCADE);
    $installer->getConnection()->createTable($table);
}
/**
 ...
 *
 */
$tableName = $installer->getTable('kir_giftregistry/item');
// Check if the table already exists
if ($installer->getConnection()->isTableExists($tableName) != true) {
    $table = $installer->getConnection()
        ->newTable($installer->getTable('kir_giftregistry/item'))
        ...
        ->addColumn('added_at', Varien_Db_Ddl_Table::TYPE_TIMESTAMP, null,
            ...
            'Added At')
        ->addIndex($installer->getIdxName('kir_giftregistry/item', array('product_id')),
            array('product_id'))
        ->addIndex($installer->getIdxName('kir_giftregistry/item', array('registry_id')),
            array('registry_id'));
    $installer->getConnection()->createTable($table);
}
$installer->endSetup();